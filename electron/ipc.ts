...
// Access the Prisma instance initialized in main.ts via a lazy getter.
// We attach it to globalThis for simplicity in this small app.
function getPrisma() {
  const prisma = (globalThis as unknown as { __PRISMA?: PrismaClient }).__PRISMA
  if (!prisma) throw new Error('Prisma not initialized')
  return prisma
}

function computeTotals(items: Array<{ quantity: number; rate: number; taxRate?: number | null; discount?: number | null }>) {
  let subtotal = 0
  let taxTotal = 0
  let discountTotal = 0
...
function nextInvoiceNumberPrefix(date = new Date()) {
  const y = date.getFullYear()
  const m = String(date.getMonth() + 1).padStart(2, '0')
  return `INV-${y}${m}`
}

async function generateInvoiceNumber(prisma: PrismaClient) {
  const prefix = nextInvoiceNumberPrefix()
  // Count existing invoices for the current month and increment.
...
ipcMain.handle('clients:create', async (_e, data: { name: string; email?: string; phone?: string; billingAddress?: string; taxId?: string }) => {
  const prisma = getPrisma()
  return prisma.client.create({ data })
})

ipcMain.handle('clients:update', async (_e, id: number, data: Partial<{ name: string; email?: string; phone?: string; billingAddress?: string; taxId?: string }>) => {
  const prisma = getPrisma()
  return prisma.client.update({ where: { id }, data })
})

ipcMain.handle('clients:get', async (_e, id: number) => {
  const prisma = getPrisma()
...
// Invoices
ipcMain.handle('invoices:list', async (_e, args?: { status?: InvoiceStatus; q?: string }) => {
  const prisma = getPrisma()
  const where: Prisma.InvoiceWhereInput = {}
  if (args?.status) where.status = args.status
  if (args?.q) {
    where.OR = [
      { number: { contains: args.q } },
      { client: { is: { name: { contains: args.q } } } },
    ]
  }
...
ipcMain.handle('invoices:get', async (_e, id: number) => {
  const prisma = getPrisma()
...
ipcMain.handle('invoices:update', async (_e, id: number, data: Partial<{ issueDate: string; dueDate: string; currency: string; notes: string; terms: string }>) => {
  const prisma = getPrisma()
  const current = await prisma.invoice.findUnique({ where: { id } })
  if (!current) throw new Error('Invoice not found')
  if (current.status === 'PAID') {
    // Allow updating notes/terms only
...
ipcMain.handle('invoices:duplicate', async (_e, id: number) => {
  const prisma = getPrisma()
...
ipcMain.handle('invoices:send', async (_e, id: number) => {
  const prisma = getPrisma()
...
// Payments
ipcMain.handle('payments:add', async (_e, data: { invoiceId: number; amount: number; date: string; mode?: string; reference?: string; note?: string }) => {
  const prisma = getPrisma()
  ...
  if (newBalance < -0.0001) throw new Error('Payment exceeds invoice total')
